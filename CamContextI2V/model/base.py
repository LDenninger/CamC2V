...
from utils.transforms import resample_poses_slerp
...
class CameraControlLVDM(DynamiCrafter):
    def __init__(self,
                 ...
                 normalize_T0=False,
                 weight_decay=1e-2,
                 camera_embedding: Literal["plucker", "ray"] = 'plucker',
                 *args,
                 **kwargs):
    ...
    def ray_condition(self, K, c2w, H, W, device, flip_flag=None):
        ...
        return_plucker = self.camera_embedding == "plucker"
        ...
        if not return_plucker:
            ray_enc = torch.cat((rays_o, rays_d), dim=-1) 
            ray_enc = ray_enc.reshape(B, c2w.shape[1], H, W, 6) 
            ray_enc = rearrange(ray_enc, "b f h w c -> b c f h w")  # [b, 6, f, h, w]
            return ray_enc
        # c2w @ dirctions
    ...
    def get_pairwise_relative_pose(self, RT_4x4_1: Tensor, RT_4x4_2: Tensor, mode='left', normalize_T0=False):
        b, t_1, _, _ = RT_4x4_1.shape  # b,t,4,4
        b, t_2, _, _ = RT_4x4_2.shape  # b,t,4,4
        ...
        RT_4x4_1 = repeat(RT_4x4_1, 'B T H W -> B T C H W', C=t_2)
        RT_4x4_2 = repeat(RT_4x4_2, 'B T H W -> B C T H W', C=t_1)

        if mode=='left':
            relative_RT_4x4 = RT_4x4_1.inverse() @ RT_4x4_2
        elif mode=='right':
            relative_RT_4x4 = RT_4x4_1 @ RT_4x4_2.inverse()

        return relative_RT_4x4
    
    def correct_batch(self, batch, video_length=16):
        prepared_batch = {}
        if 'video' in batch:
            prepared_batch['video'] = batch['video'][:, :, :video_length]
        if 'RT' in batch:
            prepared_batch['RT'] = batch['RT'][:, :video_length]
        if 'camera_data' in batch:
            prepared_batch['camera_data'] = batch['camera_data'][:, :video_length]
        if 'camera_intrinsics' in batch:
            prepared_batch['camera_intrinsics'] = batch['camera_intrinsics'][:, :video_length]
        
        for key in ['caption', 'video_path', 'fps', 'frame_stride', 'cond_frames', 'RT_cond']:
            if key in batch:
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False,
        ...
        if num_frames > 16:
            batch = self.correct_batch(batch)
            x = super().get_input(batch, self.first_stage_key)
            num_frames = 16
        ...
        **kwargs,
        ...
        if sample:
            ...
            pre_process_log, pre_process_kwargs = self.log_images_sample_log_pre_process(
                batch_, z, x, cond_frame_index, trace_scale_factor, **kwargs
            )
            ...
            with self.ema_scope("Plotting"):
                samples, z_denoise_row = self.sample_log(cond=c, batch_size=N, ddim=use_ddim,
                                                        ...
                                                        enable_camera_condition=enable_camera_condition,
                                                        latents_dir=latents_dir, **kwargs)
