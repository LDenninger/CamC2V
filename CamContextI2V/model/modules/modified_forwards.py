...
# add RT input to forward of unet
def new_forward_for_unet_pose_guided_enc(self, x, timesteps, context=None, features_adapter=None, fs=None, camera_condition=None, **kwargs):
    b, _, t, _, _ = x.shape
    t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False).type(x.dtype)
    emb = self.time_embed(t_emb)
...
def new__forward_for_BasicTransformerBlock_of_TemporalTransformer(self, x, context=None, mask=None, camera_condition=None, multi_image_condition=None):
