...
class Resampler(nn.Module):
    def __init__(
        ...
        ff_mult=4,
        video_length=None,
        use_timestep_emb=False, # using frame-wise version or not
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.use_timestep_emb:
            t_emb = timestep_embedding(torch.arange(self.video_length), self.dim, repeat_only=False).to(dtype=latents.dtype, device=latents.device)
